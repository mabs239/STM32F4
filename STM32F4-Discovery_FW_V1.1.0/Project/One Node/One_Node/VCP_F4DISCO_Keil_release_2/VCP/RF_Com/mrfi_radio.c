...
/* GDO functionality */
#define MRFI_GDO_FIFOTHR        1
#define MRFI_GDO_SYNC           6
#define MRFI_GDO2_SYNC          7
...
#define MRFI_SETTING_PATABLE0       0xCB
...
#define MRFI_SETTING_IOCFG2     MRFI_GDO2_SYNC
 ...
 */
#define MRFI_SETTING_MCSM1      0x3F // MT original setting was 0x3C
...
#define MRFI_SETTING_FIFOTHR    (0x01 | (SMARTRF_SETTING_FIFOTHR & (BV(4)|BV(5)|BV(6))))
 ...
 */
#define MRFI_MAX_DELAY_US 10 /* usec */
...
#define MRFI_ENABLE_SYNC_PIN_INT()                  MRFI_ENABLE_GDO2_INT()
...
#define MRFI_CONFIG_SYNC_PIN_RISING_EDGE_INT()      EXTILine0_Config()//MRFI_CONFIG_GDO2_RISING_EDGE_INT()
...
#define MRFI_CONFIG_GDO2_AS_PAPD_SIGNAL()           mrfiSpiWriteReg(IOCFG2, MRFI_GDO_PA_PD)
#define MRFI_CONFIG_GDO2_AS_SYNC_SIGNAL()           mrfiSpiWriteReg(IOCFG2, MRFI_GDO2_SYNC)
...
{                                                                             \
  int16 delay = MRFI_RSSI_VALID_DELAY_US;                                   \
  do                                                                          \
  {                                                                           \
...
//extern unsigned char USB_Packet_Time[23]; // added by MT
extern char rx_flag;
/* ------------------------------------------------------------------------------------------------
 *                                    Local Constants
 * ------------------------------------------------------------------------------------------------
 */
static const uint8 mrfiRadioCfg[][2] =
{
...
static void Mrfi_DelayUsec(uint32_t howLong);
static void Mrfi_DelayUsecSem(uint16 howLong);
static int8 Mrfi_CalculateRssi(uint8 rawValue);
 ...
 * ------------------------------------------------------------------------------------------------
 */
static uint8 mrfiRadioState  = MRFI_RADIO_STATE_UNKNOWN;
static mrfiPacket_t mrfiIncomingPacket;
static uint8 mrfiRndSeed;

/* reply delay support */
static volatile uint8  sReplyDelayContext;
static volatile uint8  sKillSem;
static          uint16 sReplyDelayScalar;
static          uint16 sBackoffHelper;
...
#define   PHY_PREAMBLE_SYNC_BYTES     8
  ...
  {
    uint32_t dataRate, bits, temp_exponent;
    ...
    /* we can now get data rate */
	 temp_exponent = MRFI_RADIO_OSC_FREQ/exponent;
    dataRate = ((uint32_t) mantissa) * (temp_exponent);
   ...
   */
  MRFI_CONFIG_GDO2_AS_SYNC_SIGNAL();
...
uint8 MRFI_Transmit(mrfiPacket_t * pPacket, uint8 txType)
{
  ...
  else if( txType == MRFI_TX_TYPE_CCA )
  {
     ...
     */
    MRFI_CONFIG_GDO2_AS_PAPD_SIGNAL();
 ...
 // uint8 lowdigit, *arr_ptr=0;
  ...
  else
  {
    ...
    if ((rxBytes != (frameLen + MRFI_LENGTH_FIELD_SIZE + MRFI_RX_METRICS_SIZE)) ||
        ...
        (frameLen < (MRFI_MIN_SMPL_FRAME_SIZE - 4))	// MT added '-4' to receive 8 byte frame 
       )
    {
      mrfiSpiIState_t s;
       ...
       */
      //MCU_ENTER_CRITICAL_SECTION(s);
      MRFI_STROBE_IDLE_AND_WAIT();
      mrfiSpiCmdStrobe( SFRX );
      mrfiSpiCmdStrobe( SRX );
     // MCU_EXIT_CRITICAL_SECTION(s);
...
/*            
            rxBytes = mrfiIncomingPacket.frame[5];
			
				arr_ptr = USB_Packet_Time;

				if(rxBytes == 0x79){
					USB_Packet_Time[2] = 0x31;
					rx_flag = 1;
				}
				
				else if (rxBytes == 0x7A){
					USB_Packet_Time[2] = 0x32;
					rx_flag = 1;
				}
				
				else if (rxBytes == 0x7B){
					USB_Packet_Time[2] = 0x33;
					rx_flag = 1;
				}

				else if (rxBytes == 0x7C){
					USB_Packet_Time[2] = 0x34;
					rx_flag = 1;
				}
				...
				rxBytes = mrfiIncomingPacket.frame[12];
				*(arr_ptr + 6) =	0x30 + (rxBytes & 0x0F);
				*(arr_ptr + 5) =	0x30 + ((rxBytes >> 4) & 0x03);

				rxBytes = mrfiIncomingPacket.frame[13];
				*(arr_ptr + 9) =	0x30 + (rxBytes & 0x0F);
				*(arr_ptr + 8) =	0x30 + (rxBytes >> 4);

				rxBytes = mrfiIncomingPacket.frame[14];
				*(arr_ptr + 12) =	0x30 + (rxBytes & 0x0F);
				*(arr_ptr + 11) =	0x30 + (rxBytes >> 4);

				rxBytes = mrfiIncomingPacket.frame[15];
				lowdigit = (char)(rxBytes % 10);
				*(arr_ptr + 18) =	0x30 + lowdigit;
				*(arr_ptr + 17) =	0x30 + (rxBytes - lowdigit)/10;

				rxBytes = mrfiIncomingPacket.frame[17];
				lowdigit = (char)(rxBytes % 10);
				*(arr_ptr + 15) =	0x30 + lowdigit;
				*(arr_ptr + 14) =	0x30 + (rxBytes - lowdigit)/10;
...
static int8 Mrfi_CalculateRssi(uint8 rawValue)
{
  ...
  if(rawValue >= 128)
  {
    rssi = (int16)(rawValue - 256)/2 - MRFI_RSSI_OFFSET;
  }
  else
  {
 ...
 **************************************************************************************************
 */
void Mrfi_RandomBackoffDelay(void)
{
  uint8 backoffs;
  uint8 i;
...
{
  mrfiSpiIState_t s;
  uint16 count = howLong/MRFI_MAX_DELAY_US;
  if (howLong*1000)
  {
    do
    {
     // MCU_ENTER_CRITICAL_SECTION(s);
        //MCU_DELAY_CYCLES(MRFI_MAX_DELAY_US)
...
{
  mrfiSpiIState_t s;
  uint16    milliseconds = sReplyDelayScalar;
